...
def startup():
    ...
    try:
        ...
        _expiration_scheduler.add_job(_run_daily_expiration_reminders, "cron", hour=hour, minute=minute,
...
def list_items(
        ...
        sort_by: Optional[str] = Query("created_at",
                                       description="Sort field: name, expiration_date, created_at, quantity"),
    ...
    try:
        ...
        if household_id:
            ...
            member_check = supabase.table("relation_househould").select("*").eq("user_id", user_id).eq("household_id",
            ...
            household_response = supabase.table("relation_househould").select("household_id").eq("user_id",
                                                                                                 user_id).limit(
                1).execute()
        ...
        members_response = supabase.table("relation_househould").select("user_id").eq("household_id",
                                                                                      target_household_id).execute()
        ...
        if expiring_soon is True:
            ...
            query = query.not_.is_("expiration_date", "null").gte("expiration_date", today.isoformat()).lte(
        ...
        logger.info(
            f"Retrieved {len(response.data)} items (page {page}/{total_pages}, total: {total}) for user: {user_id}")
...
def create_item(item_data: ItemCreate, request: Request, user_id: Optional[str] = Depends(get_user_id)):
    ...
    try:
        ...
        logger.info(
            f"Creating item '{item_data.name}' (qty: {item_data.quantity}, storage: {storage_type}, opened: {is_opened}) for user: {user_id}")
        new_item = {
            "user_id": user_id,
            "name": item_data.name,
...
def delete_item(item_id: str, request: Request, user_id: Optional[str] = Depends(get_user_id)):
    ...
    try:
        ...
        if item.get("expiration_date"):
            exp_date = datetime.fromisoformat(item["expiration_date"].split("T")[0]).date() if isinstance(
                item["expiration_date"], str) else item["expiration_date"]
        ...
        try:
            ...
            logger.info(
                f"Logged deleted item {item_id} to deleted_items (expired: {was_expired}, expiring soon: {was_expiring_soon})")
        except Exception as e:
...
def get_expiring_items(
    ...
    try:
        # Build query
        query = supabase.table("items").select("*", count="exact").eq("user_id", user_id).not_.is_("expiration_date",
                                                                                                   "null").gte(
            "expiration_date", today.isoformat()).lte("expiration_date", future_date.isoformat()).order(
            "expiration_date")
        ...
        logger.info(
            f"Found {len(response.data)} items expiring within {days} days (page {page}/{total_pages}, total: {total}) for user: {user_id}")
...
EXPIRATION_RULES = {
    ...
    "dairy": {
        "keywords": [
            # Milk and cream
            "milk", "whole milk", "2% milk", "1% milk", "skim milk", "nonfat milk", "cream", "heavy cream",
            "whipping cream", "light cream", "half and half", "buttermilk", "evaporated milk", "condensed milk",
            "sweetened condensed milk",
            # Yogurt
            "yogurt", "greek yogurt", "plain yogurt", "vanilla yogurt", "yogurt drink", "kefir", "drinkable yogurt",
            # Cheese (general)
            "cheese", "cheddar", "swiss", "gouda", "brie", "feta", "parmesan", "mozzarella", "ricotta",
            "cottage cheese", "cream cheese", "mascarpone", "sour cream", "butter", "salted butter", "unsalted butter",
            "european butter",
            # Additional cheeses
            "provolone", "monterey jack", "colby", "pepper jack", "muenster", "havarti", "fontina", "asiago",
            "pecorino", "romano", "manchego", "gruyere", "emmental", "camembert", "goat cheese", "chevre",
            "blue cheese", "gorgonzola", "roquefort", "stilton", "cheddar cheese", "swiss cheese",
            # Processed cheese
    ...
    "dairy_soft": {
        "keywords": ["cream cheese", "cottage cheese", "ricotta", "mozzarella", "brie", "camembert", "goat cheese",
                     "fresh cheese"],
        "pantry": None,
        "fridge": 5,  # 5 days
        "freezer": 60,  # 2 months
    ...
    "meat": {
        "keywords": [
            # Poultry
            "chicken", "turkey", "duck", "goose", "cornish hen", "chicken breast", "chicken thigh", "chicken wing",
            "chicken leg", "chicken drumstick", "whole chicken", "chicken parts",
            # Beef
            "beef", "steak", "ribeye", "sirloin", "tenderloin", "filet mignon", "strip steak", "t-bone", "porterhouse",
            "flank steak", "skirt steak", "brisket", "roast beef", "beef roast", "chuck roast", "pot roast",
            "beef stew meat", "beef tips",
            # Pork
            "pork", "pork chop", "pork tenderloin", "pork loin", "pork shoulder", "pork butt", "pork ribs",
            "baby back ribs", "spare ribs", "country ribs", "pork belly",
            # Lamb and other
            "lamb", "lamb chop", "lamb leg", "lamb shoulder", "veal", "venison", "bison", "buffalo", "elk", "rabbit",
            # Processed meats
            "sausage", "bacon", "ham", "deli meat", "lunch meat", "cold cuts", "salami", "pepperoni", "prosciutto",
            "pancetta", "chorizo", "andouille", "bratwurst", "italian sausage", "breakfast sausage", "hot dog",
            "hotdogs", "frankfurter", "wieners",
            # Ground meats
            "ground beef", "ground turkey", "ground pork", "ground chicken", "ground lamb", "ground meat", "mince",
            "meatballs",
        ],
    ...
    "meat_ground": {
        "keywords": ["ground beef", "ground turkey", "ground pork", "ground chicken", "ground lamb", "ground meat",
                     "mince"],
        "pantry": None,
        "fridge": 2,  # 2 days
        "freezer": 90,  # 3 months
    ...
    "seafood": {
        "keywords": [
            # Fish
            "fish", "salmon", "tuna", "cod", "tilapia", "halibut", "mackerel", "sardines", "anchovies", "sea bass",
            "trout", "catfish", "snapper", "grouper", "swordfish", "mahi mahi", "bass", "perch", "walleye", "pike",
            "flounder", "sole", "pollock", "haddock", "whiting", "rockfish", "red snapper", "yellowtail", "branzino",
            "arctic char", "sturgeon",
            # Shellfish
            "shrimp", "crab", "lobster", "oysters", "mussels", "clams", "scallops", "crayfish", "crawfish",
            "langoustine", "prawns", "king crab", "snow crab", "dungeness crab", "blue crab", "stone crab",
            # Other seafood
    ...
    "produce_perishable": {
        "keywords": [
            # Leafy greens
            "lettuce", "spinach", "kale", "arugula", "chard", "collard greens", "mustard greens", "bok choy",
            "napa cabbage", "swiss chard", "watercress", "endive", "frisée", "radicchio", "mache", "lambs lettuce",
            # Vegetables
            "broccoli", "carrots", "celery", "bell pepper", "cucumber", "tomato", "mushrooms", "asparagus",
            "green beans", "zucchini", "squash", "eggplant", "cauliflower", "brussels sprouts", "cabbage", "radishes",
            "turnips", "beets", "corn on the cob",
            # Peppers (vegetables)
            "bell pepper", "red pepper", "green pepper", "red bell pepper", "green bell pepper", "yellow pepper",
            "orange pepper", "yellow bell pepper", "orange bell pepper", "sweet pepper", "jalapeño", "jalapeno",
            "serrano", "habanero", "poblano", "anaheim", "banana pepper", "chili pepper", "chile pepper",
            # Berries and small fruits
            "berries", "grapes", "strawberries", "blueberries", "raspberries", "blackberries", "cherries",
            "cranberries", "gooseberries", "currants", "elderberries", "mulberries",
            # Other perishable produce
            "artichoke", "artichokes", "fennel", "leeks", "scallions", "green onions", "spring onions", "shallots",
            "okra", "pattypan squash", "yellow squash", "summer squash", "acorn squash", "butternut squash",
            "spaghetti squash", "delicata squash",
        ],
    ...
    "produce_leafy": {
        "keywords": ["lettuce", "spinach", "kale", "arugula", "chard", "collard greens", "mustard greens", "mesclun",
                     "spring mix", "baby spinach", "romaine", "iceberg"],
        "pantry": None,
        "fridge": 5,  # 5 days
        "freezer": 30,
    },
    ...
    "produce_berries": {
        "keywords": ["strawberries", "blueberries", "raspberries", "blackberries", "cranberries", "gooseberries",
                     "currants"],
        "pantry": None,
        "fridge": 5,  # 5 days
        "freezer": 180,  # 6 months
    ...
    "produce_long": {
        "keywords": [
            # Root vegetables
            "potato", "onion", "garlic", "sweet potato", "yam", "ginger", "shallot", "leek", "turnip", "rutabaga",
            "parsnip", "beet", "carrot", "daikon", "radish", "jicama", "celeriac", "celery root",
            # Apples and pears
            "apple", "pear", "granny smith", "gala", "fuji", "honeycrisp", "red delicious", "golden delicious",
            "bosc pear", "anjou pear", "bartlett pear",
            # Citrus
            "orange", "lemon", "lime", "grapefruit", "tangerine", "clementine", "mandarin", "blood orange", "cara cara",
            "satsuma", "pomelo", "yuzu", "kumquat",
            # Stone fruits
            "banana", "plum", "peach", "nectarine", "apricot", "cherry", "sweet cherry", "sour cherry",
            # Tropical fruits
            "avocado", "mango", "pineapple", "watermelon", "cantaloupe", "honeydew", "kiwi", "papaya", "guava",
            "passion fruit", "dragon fruit", "lychee", "rambutan", "starfruit", "carambola", "persimmon", "pomegranate",
    ...
    "produce_root": {
        "keywords": ["potato", "sweet potato", "yam", "onion", "garlic", "shallot", "ginger", "turnip", "rutabaga",
                     "parsnip", "beet", "carrot"],
        "pantry": 60,  # 60 days
        "fridge": 30,  # 30 days
        "freezer": 180,  # 6 months
    ...
    # Bread & Bakery
    "bread": {
        "keywords": ["bread", "bagel", "muffin", "roll", "bun", "croissant", "biscuit", "danish", "donut", "doughnut",
                     "pastry", "pita", "tortilla", "naan", "flatbread", "sourdough", "rye bread", "wheat bread",
    ...
    # Canned goods
    "canned": {
        "keywords": ["can", "canned", "soup", "beans", "corn", "peas", "tuna can", "salmon can", "sardines can",
                     "tomatoes can", "tomato paste", "tomato sauce", "broth", "stock", "canned fruit",
    ...
    "dry": {
        "keywords": ["pasta", "rice", "flour", "cereal", "oats", "quinoa", "lentils", "beans dry", "black beans",
                     "kidney beans", "chickpeas", "garbanzo", "barley", "bulgur", "couscous", "buckwheat", "millet",
                     "farro", "wild rice", "brown rice", "white rice", "basmati", "jasmine rice", "breadcrumbs",
                     "cornmeal", "yeast", "active dry yeast", "instant yeast"],
    ...
    # Snacks & Packaged
    "packaged": {
        "keywords": ["chips", "crackers", "cookies", "nuts", "pretzels", "popcorn", "trail mix", "granola",
                     "granola bar", "protein bar", "energy bar", "dried fruit", "raisins", "dates", "prunes", "figs",
                     "apricots dried"],
        "pantry": 90,  # 3 months
        "fridge": 90,
        "freezer": 180,  # 6 months
    ...
    "nuts": {
        "keywords": ["almonds", "walnuts", "pecans", "cashews", "peanuts", "pistachios", "hazelnuts", "macadamia",
                     "brazil nuts", "pine nuts", "sunflower seeds", "pumpkin seeds", "chia seeds", "flax seeds",
                     "sesame seeds"],
        "pantry": 180,  # 6 months
        "fridge": 365,  # 1 year
        "freezer": 365,  # 1 year
    ...
    # Beverages
    "beverages": {
        "keywords": ["juice", "soda", "water", "coffee", "tea", "lemonade", "iced tea", "sports drink", "energy drink",
                     "milk alternative", "almond milk", "soy milk", "oat milk", "coconut milk", "orange juice",
    ...
    "condiments": {
        "keywords": ["ketchup", "mustard", "relish", "pickles", "olives", "capers", "salsa", "hot sauce",
                     "barbecue sauce", "ranch", "italian dressing", "vinaigrette", "ranch dressing", "caesar dressing",
                     "thousand island", "tartar sauce", "cocktail sauce", "taco sauce", "enchilada sauce"],
        "pantry": 180,  # 6 months (unopened)
    ...
    # Preserves & Spreads
    "preserves": {
        "keywords": ["jam", "jelly", "preserves", "marmalade", "fruit spread", "peanut butter", "almond butter",
                     "cashew butter", "sunflower butter", "nutella", "chocolate spread"],
    ...
    "oils": {
        "keywords": ["olive oil", "vegetable oil", "canola oil", "coconut oil", "sesame oil", "avocado oil",
                     "grapeseed oil", "sunflower oil", "safflower oil", "peanut oil"],
    ...
    "spices": {
        "keywords": [
            ...
            "tarragon", "marjoram", "chervil", "sumac",
            # Ground spices
            "paprika", "cumin", "coriander", "turmeric", "cinnamon", "nutmeg", "cloves", "allspice", "cardamom",
            "star anise", "bay leaves",
            # Pepper varieties (spices)
            "black pepper", "white pepper", "red pepper", "green pepper", "red pepper flakes", "red pepper powder",
            "green pepper flakes", "cayenne pepper", "cayenne", "crushed red pepper", "pink peppercorns",
            "green peppercorns", "szechuan pepper", "sichuan pepper", "ground pepper", "peppercorns",
            # Seeds
            "sesame seeds", "poppy seeds", "fennel seeds", "caraway seeds", "celery seeds", "mustard seeds",
            "cumin seeds", "coriander seeds", "anise seeds", "nigella seeds", "black seeds",
            # Indian spices
            "curry powder", "garam masala", "tandoori masala", "chaat masala", "fenugreek", "asafoetida", "hing",
            "ajwain", "carom seeds", "kalonji",
            # Middle Eastern spices
            "za'atar", "zaatar", "sumac", "baharat", "ras el hanout", "harissa", "dukkah",
            # Asian spices
            "five spice", "five-spice powder", "szechuan peppercorns", "sichuan peppercorns", "sansho pepper", "sansho",
            "shichimi togarashi", "togarashi",
            # Latin American spices
            "achiote", "annatto", "adobo", "sazon", "sazonador",
            # Specific ground spices
            "ground ginger", "ground cinnamon", "ground nutmeg", "ground allspice", "ground cloves", "ground cardamom",
            "ground coriander", "ground cumin", "ground turmeric", "ground paprika",
            # Whole spices
            "whole cloves", "whole cardamom", "whole allspice", "whole nutmeg", "cinnamon sticks", "cinnamon bark",
            "vanilla beans", "vanilla pods",
            # Chili powders and peppers
            "chili powder", "chile powder", "chipotle powder", "ancho chili powder", "guajillo powder",
            "smoked paprika", "pimenton", "aleppo pepper", "urfa biber",
            # Specialty blends
            "herbes de provence", "italian seasoning", "poultry seasoning", "pumpkin pie spice", "apple pie spice",
            "chai spice", "berbere", "ras el hanout",
            # Salt blends (spice blends with salt)
            "seasoned salt", "garlic salt", "onion salt", "celery salt", "lemon pepper", "cajun seasoning",
            "creole seasoning", "old bay", "old bay seasoning",
            # Other common spices
            "garlic powder", "onion powder", "chili flakes", "red chili flakes", "smoked salt", "hickory smoked salt",
            "liquid smoke",
            # Additional herbs
            "dried basil", "dried oregano", "dried thyme", "dried rosemary", "dried sage", "dried parsley",
            "dried dill", "dried mint", "dried tarragon", "dried marjoram",
    ...
    "herbs_fresh": {
        "keywords": ["fresh basil", "fresh oregano", "fresh thyme", "fresh rosemary", "fresh parsley", "fresh cilantro",
                     "fresh dill", "fresh mint", "fresh chives", "fresh sage"],
        "pantry": None,
        "fridge": 7,  # 7 days
        "freezer": 180,  # 6 months
    },
    ...
    "pantry_staples": {
        "keywords": [
            # Sugars
            "sugar", "white sugar", "brown sugar", "powdered sugar", "confectioners sugar", "granulated sugar",
            "cane sugar", "raw sugar", "turbinado sugar", "demerara sugar",
            # Salts
            "salt", "table salt", "sea salt", "kosher salt", "himalayan salt", "rock salt", "iodized salt",
            # Baking ingredients
            "baking powder", "baking soda", "cream of tartar", "vanilla extract", "almond extract", "vanilla bean",
            "cornstarch", "arrowroot", "gelatin", "pudding mix", "cake mix", "brownie mix",
            # Long-lasting condiments/sauces
            "soy sauce", "fish sauce", "oyster sauce", "hoisin sauce", "teriyaki sauce", "worcestershire sauce",
            "tabasco", "sriracha", "chili sauce", "vinegar", "balsamic", "rice vinegar", "apple cider vinegar",
            "white vinegar", "red wine vinegar",
            # Sweeteners
            "honey", "maple syrup", "agave", "molasses", "corn syrup", "golden syrup",
            # Other long-lasting items
            "cocoa powder", "unsweetened cocoa", "chocolate chips", "chocolate bar", "coconut flakes",
            "shredded coconut", "dried coconut",
        ],
    ...
    # Frozen foods
    "frozen": {
        "keywords": ["frozen", "ice cream", "frozen vegetables", "frozen fruit", "frozen meal", "frozen pizza",
                     "frozen burrito", "frozen waffles", "frozen berries"],
        "pantry": None,
        "fridge": None,
        "freezer": 180,  # 6 months
    },
    # Deli & Prepared foods
    "prepared": {
        "keywords": ["deli", "prepared", "ready meal", "meal kit", "salad kit", "hummus", "guacamole", "pesto",
                     "tzatziki", "dip"],
        "pantry": None,
        "fridge": 5,  # 5 days
        "freezer": 30,  # 1 month
    ...
    "kitchen_cleaning": {
        "keywords": ["dish soap", "sponge", "scrubber", "paper towels", "napkins", "trash bags", "garbage bags",
                     "aluminum foil", "plastic wrap", "wax paper", "parchment paper", "ziploc", "sandwich bag"],
...
def suggest_expiration_date(
    ...
    cleaned_name = re.sub(r'\b(organic|fresh|frozen|dried|raw|cooked|whole|low fat|fat free|reduced fat|light|lite)\b',
                          '', item_name_lower)
    ...
    vegetable_context_words = ["bell", "fresh", "sweet", "chili pepper", "chile pepper", "jalapeño", "jalapeno",
                               "serrano", "habanero", "poblano"]
    ...
    if matched_category is not None and matched_days is None:
        ...
        if rec_storage and rec_storage != storage_type:
            if is_less_safe_storage(storage_type, rec_storage):
                ...
                highly_perishable = ["meat", "meat_ground", "seafood", "seafood_shellfish", "dairy", "dairy_soft",
                                     "eggs"]
    ...
    if matched_days is not None:
        ...
        if is_opened and matched_days is not None:
            ...
            elif matched_category in ["dairy", "meat", "seafood", "produce_perishable", "produce_leafy",
                                      "produce_berries"]:
...
def change_password(password_data: PasswordChangeRequest, request: Request,
                    user_id: Optional[str] = Depends(get_user_id)):
...
def get_notification_preferences(user_id: Optional[str] = Depends(get_user_id)):
    ...
    if not user_id:
        raise HTTPException(status_code=401, detail="Authentication required")
    try:
        response = supabase.table("expiration_notification_preferences").select("channel", "contact").eq("user_id",
                                                                                                         user_id).limit(
            1).execute()
        if not response.data:
...
def update_notification_preferences(data: NotificationPreferencesUpdate, user_id: Optional[str] = Depends(get_user_id)):
    ...
    try:
        existing = supabase.table("expiration_notification_preferences").select("user_id").eq("user_id", user_id).limit(
            1).execute()
...
def _send_expiration_reminders_for_user(user_id: str, days: int = 7) -> Tuple[bool, str]:
    ...
    future_date = today + timedelta(days=days)
    try:
        prefs_response = supabase.table("expiration_notification_preferences").select("user_id", "channel",
                                                                                      "contact").eq("user_id",
                                                                                                    user_id).limit(
            1).execute()
        ...
        items_response = supabase.table("items").select("id", "name", "expiration_date").eq("user_id",
                                                                                            user_id).not_.is_(
            "expiration_date", "null").gte("expiration_date", today.isoformat()).lte("expiration_date",
                                                                                     future_date.isoformat()).order(
            "expiration_date").execute()
...
def send_expiration_reminders(
    ...
    try:
        ...
        prefs_response = supabase.table("expiration_notification_preferences").select("channel").eq("user_id",
                                                                                                    user_id).limit(
            1).execute()
        channel = (
            prefs_response.data[0].get("channel") if prefs_response.data and len(prefs_response.data) > 0 else None)
...
def get_profile_stats(user_id: Optional[str] = Depends(get_user_id)):
    ...
    try:
        ...
        total_items = items_response.count if hasattr(items_response,
                                                      'count') and items_response.count is not None else len(
            items_response.data)
        ...
        today = date.today()
        future_date = today + timedelta(days=7)
        expiring_response = supabase.table("items").select("id", count="exact").eq("user_id", user_id).not_.is_(
            "expiration_date", "null").gte("expiration_date", today.isoformat()).lte("expiration_date",
                                                                                     future_date.isoformat()).execute()
        expiring_items = expiring_response.count if hasattr(expiring_response,
                                                            'count') and expiring_response.count is not None else len(
            expiring_response.data)

        # Get expired items count
        expired_response = supabase.table("items").select("id", count="exact").eq("user_id", user_id).not_.is_(
            "expiration_date", "null").lt("expiration_date", today.isoformat()).execute()
        expired_items = expired_response.count if hasattr(expired_response,
                                                          'count') and expired_response.count is not None else len(
...
async def get_recipes_by_ingredients(
        ...
        household_id: Optional[str] = Query(None,
                                            description="Household ID for expiring-soon lookup when prioritize_expiring is true"),
):
    ...
    try:
        ...
        if prioritize_expiring and recipes_out:
            try:
                if household_id:
                    member_check = supabase.table("relation_househould").select("*").eq("user_id", user_id).eq(
                        "household_id", household_id).execute()
                    if not member_check.data:
                        household_id = None
                if not household_id:
                    hh = supabase.table("relation_househould").select("household_id").eq("user_id", user_id).limit(
                        1).execute()
                    household_id = hh.data[0]["household_id"] if hh.data else None
                if household_id:
                    members = supabase.table("relation_househould").select("user_id").eq("household_id",
                                                                                         household_id).execute()
                    user_ids = [m["user_id"] for m in members.data]
                    today = date.today()
                    from datetime import timedelta
                    soon = today + timedelta(days=7)
                    expiring = supabase.table("items").select("name").in_("user_id", user_ids).not_.is_(
                        "expiration_date", "null").gte("expiration_date", today.isoformat()).lte("expiration_date",
                                                                                                 soon.isoformat()).execute()
                    expiring_names = {(i.get("name") or "").strip().lower() for i in (expiring.data or []) if
                                      (i.get("name") or "").strip()}
        ...
        for item in items:
            ...
            try:
                ...
                if suggested_date:
                    ...
                    logger.info(
                        f"Suggested expiration for '{new_item['name']}': {suggested_date} (confidence: {confidence}, storage: {storage_for_calculation})")
                else:
    ...
    try:
        ...
        if json_match:
            ...
            scan_sessions[token]["result"] = {
                "error": "Could not parse receipt data. The image may be unclear or not contain a valid receipt."}
            raise HTTPException(
                status_code=422,
                detail="Could not parse receipt data. The image may be unclear or not contain a valid receipt. Please try with a clearer image."
            )
...
def get_user_households(user_id: Optional[str] = Depends(get_user_id)):
    ...
    try:
        response = supabase.table("relation_househould").select("household_id, household(id, name)").eq("user_id",
                                                                                                        user_id).execute()
        households = [{"id": r["household"]["id"], "name": r["household"]["name"]} for r in response.data if
                      r.get("household")]
...
def join_household(
    ...
    try:
        ...
        existing = supabase.table("relation_househould").select("*").eq("user_id", user_id).eq("household_id",
                                                                                               request.household_id).execute()
        if existing.data:
...
def get_household_members(
    ...
    try:
        ...
        # Get all members
        members_response = supabase.table("relation_househould").select("user_id, profiles(name, email)").eq(
            "household_id", household_id).execute()
        members = [{"id": m["user_id"], "name": m["profiles"]["name"], "email": m["profiles"]["email"]} for m in
                   members_response.data if m.get("profiles")]
...
def list_all_users():
    ...
    try:
        ...
        try:
            ...
            profile_users = [{"id": p["id"], "email": p["email"], "name": p["name"], "source": "profiles"} for p in
                             profiles.data]
        except Exception as e:
...
async def get_nutrition(
    ...
    """
    Get nutrition facts for a pantry item.
    Looks up the item by ID to get its name, then searches USDA FoodData Central.
    """
    if not user_id:
        raise HTTPException(status_code=401, detail="Authentication required")
    ...
    # 1. Fetch the item from DB to get its name (and usda_fdc_id if stored)
    try:
        item_response = (
    ...
    item_name = item.get("name", "")
    stored_fdc_id = item.get("usda_fdc_id")

    async with httpx.AsyncClient(timeout=10.0) as client:
        # 2. If we have a stored FDC ID, fetch directly; otherwise search by name
        fdc_id = stored_fdc_id
        usda_name = item_name
        ...
        if not fdc_id:
            try:
                search_url = (
                    ...
                    f"?query={item_name}&pageSize=1&api_key={USDA_API_KEY}"
                )
                ...
                if not foods:
                    raise HTTPException(
                        status_code=404,
                        detail=f"No nutrition data found for '{item_name}'"
                    )
                fdc_id = foods[0].get("fdcId")
                usda_name = foods[0].get("description", item_name)
            except HTTPException:
                raise
            except Exception as e:
                logger.error(f"USDA search failed for '{item_name}': {str(e)}")
                raise HTTPException(status_code=502, detail="Nutrition data unavailable.")

        # 3. Fetch full nutrition details by FDC ID
        try:
            detail_url = f"https://api.nal.usda.gov/fdc/v1/food/{fdc_id}?api_key={USDA_API_KEY}"
            detail_resp = await client.get(detail_url)
            detail_data = detail_resp.json()
        except Exception as e:
            logger.error(f"USDA detail fetch failed for fdcId {fdc_id}: {str(e)}")
            raise HTTPException(status_code=502, detail="Nutrition data unavailable.")

    # 4. Map USDA nutrient IDs to our fields
    # See: https://fdc.nal.usda.gov/food-details/747447/nutrients
    NUTRIENT_MAP = {
    ...
    for nutrient in detail_data.get("foodNutrients", []):
        # labelNutrients path (SR Legacy / Foundation)
        n = nutrient.get("nutrient", {})
        nid = n.get("id") or nutrient.get("nutrientId")
        value = nutrient.get("amount") or nutrient.get("value")
        if nid in NUTRIENT_MAP and value is not None:
            nutrients_out[NUTRIENT_MAP[nid]] = value

    # Serving size
    serving_size = None
    if detail_data.get("servingSize") and detail_data.get("servingSizeUnit"):
        serving_size = f"{detail_data['servingSize']} {detail_data['servingSizeUnit']}"
